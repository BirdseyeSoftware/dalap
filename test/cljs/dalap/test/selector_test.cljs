...
;;
;; test/clj/dalap/test/selector_test.clj @ Tue Nov 13 19:42:11 UTC 2012
;;
(ns dalap.test.selector-test (:require [buster-cljs.core :refer [is]] [dalap.selector :as selector] [dalap.selector :refer [-gen-rules-decorator gen-rules-visitor]] [dalap.walk :refer [walk]]) (:require-macros [buster-cljs.macros :refer [deftest it initialize-buster]]))
(do (initialize-buster))
(defrecord CustomType [a b])
(defrecord CustomType2 [a b])
(defrecord CustomType3 [a b])
(deftype CustomType4 [a b])
(defn visit-clj-form [form w] (letfn [(filter-map [f form] (remove (fn* [p1__5457#] (= p1__5457# (keyword "dalap/form"))) (map f form)))] (cond (list? form) (apply list (filter-map w form)) (seq? form) (doall (filter-map w form)) (coll? form) (into (empty form) (filter-map w form)) :else form)))
